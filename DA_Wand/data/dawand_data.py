import os
from DA_Wand.util.util import is_mesh_file, pad, compute_hks
import torch
from . base_dataset import BaseDataset
import numpy as np
from DA_Wand.models.layers.meshing import Mesh
from DA_Wand.models.layers.meshing.analysis import computeDihedrals, computeFaceAreas, computeEdgeRatios, computeOppositeAngles, computeVertexNormals
from DA_Wand.models.layers.meshing.analysis import computeEdgeNeighborMatrix, computeFaceNeighborMatrix, computeFaceNormals, computeHKS
from DA_Wand.models.layers.meshing.io import PolygonSoup
from pathlib import Path
from DA_Wand.util.diffusion_net.geometry import get_operators
...
class DAWandData(BaseDataset):
    def __init__(self, opt, meshfile):
        ...
        self.paths, self.cachepaths, self.anchorcachepaths, self.operatorpaths, \
            self.labelpaths, self.anchor_fs, self.anchor_fs_labels, self.augs = \
                self.make_dataset(self.test_dir, meshfile)
    ...
    def update_anchor(self, anchor_fs):
        ...
        self.get_mean_std()
        self.opt.overwriteanchorcache = False
    ...
    def __getitem__(self, index):
        ...
        # Recompute mesh values only if cache doesn't exist or overwritecache
        cacheexists = False
        ...
        frames, mass, L, evals, evecs, gradX, gradY = \
            get_operators(vertices, faces, op_cache_dir=operatorpath, overwrite_cache=self.opt.overwriteopcache)
        ...
        if self.opt.extrinsic_features is not None and self.opt.extrinsic_condition_placement == "pre":
            ...
            if os.path.exists(anchorcachepath) and not self.opt.overwriteanchorcache:
                ...
                if "edgefeatures" in anchorfeatures.keys():
                    ...
                    mesh.edgefeatures = anchorfeatures['edgefeatures']

                for key, val in anchorfeatures.items():
                    setattr(mesh, key, val)

            if not anchorcache_exists or self.opt.overwriteanchorcache:
    ...
    def make_dataset(self, path, meshfile):
        assert is_mesh_file(meshfile)
        ...
        meshpath = os.path.join(path, meshfile)
        meshname = os.path.splitext(meshfile)[0]
